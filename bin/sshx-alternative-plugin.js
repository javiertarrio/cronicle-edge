...
// Alternative SSH Plugin for legacy distributions (Debian 8/9, older Ubuntu, etc.)
// This version uses more compatible shell syntax for variable export
...
// IMPROVED: More compatible variable export for legacy distributions
// - Validates variable names to ensure POSIX compliance
// - Uses single quotes around base64 to prevent expansion issues
...
let vars = Object.entries(process.env)
    ...
    .filter(([k, v]) => {
        // Validate variable name (POSIX compliant: alphanumeric + underscore, cannot start with digit)
        const varName = truncVar ? k.replace(/^SSH_/, '') : k;
        return varName && /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(varName);
    })
    .map(([key, value]) => {
        const b64 = Buffer.from(value || '').toString('base64');
        const varName = truncVar ? key.replace(/^SSH_/, '') : key;
        // Use single quotes around base64 value and double quotes for the export
        // This prevents issues with special characters in older dash/sh versions
        return `export ${varName}="$(printf '%s' '${b64}' | base64 -di)"`;
    })
    .join('\n')
...
printf '%s' '${SCRIPT_BASE64}' | base64 -di > "$temp_file"
chmod +x $temp_file
